...
function DungeonRunCard() {
  ...
  // 主题 id → 中文标题（packs 由 /api/state 下发）
  const dungeonPacks = useApp((s) => s.state?.dungeon?.packs ?? []);
  const themeTitle = (tid: string) => dungeonPacks.find((p) => p.themes.includes(tid))?.title ?? tid;

  if (!render) {
    return (
      <div className="mb-3.5 rounded-[14px] border border-arcane/40 bg-panel p-3.5">
        <p className="text-sm font-semibold text-arcane">紫金地牢</p>
        <p className="mt-1 text-xs text-muted">在大厅配置主题后进入</p>
      </div>
    );
  }
  ...
  const hasBase = run.active_themes.some((t) => BASE_THEMES.includes(t));
  const otherThemes = run.active_themes.filter((t) => !BASE_THEMES.includes(t));
  return (
    <div className="mb-3.5 rounded-[14px] border border-arcane/40 bg-panel p-3.5">
      ...
      <div className="mt-2.5 space-y-1.5 text-xs text-muted">
        <div className="flex flex-wrap gap-1.5">
          ...
          {otherThemes.map((t) => (
            <span key={t} className="rounded-md border border-arcane/50 bg-arcane/10 px-2 py-0.5 text-arcane">
              {themeTitle(t)}
            </span>
          ))}
        </div>
        ...
        <p className="text-text">意志 {bars(rs.will, 6)}</p>
        ...
        <p className="pt-1">
          第 {run.floor_index} 层 · 第 {run.room_index} 房 · 回合 {run.turn_index}
        </p>
      </div>
    </div>
  );
